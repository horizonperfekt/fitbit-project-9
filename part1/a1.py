...
def showCaloriesPerDay(df, userId, timeRange=(None, None)):
    ...
    plt.tight_layout()
    plt.savefig(PLOTPATH + f"calories_per_day_user_{userId}.png")
...
def showWeekdayFrequency(df):
    ...
    plt.tight_layout()
    plt.savefig(PLOTPATH + "weekday_frequency.png")
...
def basicInspection(df):
    ...
    plt.tight_layout()
    plt.savefig(PLOTPATH + "total_distance_per_user.png")
    if (SHOWPLOTS): plt.show()
...
def relationshipCaloriesSteps(df):
    ...
    model = smf.ols(formula="Calories ~ TotalSteps + Id", data=df).fit() 
    print(model.summary())
